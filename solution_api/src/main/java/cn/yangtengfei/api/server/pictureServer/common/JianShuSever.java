...
public class JianShuSever extends PictureService{
	...
	@Override
	public void savePictureToServer(String path) {
		try {
			log.info("==============path:{}",path);
			...
			log.info("pictureToken============:{}",JSON.toJSONString(pictureToken));
			...
			log.info("pitureUploadResponse============:{}",JSON.toJSONString(pitureUploadResponse));

			String insertUrl = pitureUploadResponse.getUrl();
			log.info("insertUrl============:{}",insertUrl);
		}catch (Exception e){
			log.error("savePictureToServer error",e);
		}
